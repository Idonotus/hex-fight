...
use crate::{cardrenderer::{assets::AssetCache, materials::RecolourMaterial}, content::{AllColorBand, SimpleCard}, engine::{Game, cards::{AssignedBand, BandSet}}};
...
impl<'a> Assetable for CardBox<'a> {
	fn generate_layers(&self, assets: Vec<crate::cardrenderer::assets::Asset>) -> Vec<Box<dyn std::any::Any>> {
...
fn test_system(
	...
	for l in layers {
		...
		let omat = match mat {
			Ok(m) => {m},
			Err(a ) => {println!("{:#?} {:#?}", a.type_id(), TypeId::of::<RecolourMaterial>()); continue;}
		};
