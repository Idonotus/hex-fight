...
impl ResponseContext {
    ...
    fn allocate_from_vec(&mut self, defaults: Vec<ContextItem>) -> Vec<usize> {
        self.predicates
            .append(&mut defaults.iter().map(ContextItem::get_predicate).collect());
        let prev = self.context_items.len();
        self.context_items
            .append(&mut defaults.into_iter().map(|c| Some(c)).collect());
...
impl Predicate<ContextPredicate> for Prompt {
    fn get_predicate(&self) -> ContextPredicate {
        match self {
            ...
            Prompt::PickCardsFromHand {
                player: _,
                references: _,
            } => ContextPredicate::CardReference,
        }
    }
}
